/* 일자    : 2018-07-25
 * 작성자   : HJ-SON(R&D 개발2팀), HK-CHO(R&D Hardware)
 * NOTI   : 비콘연동 AUTO DOOR SYSTEM
 * USE    : Bluetooth LE device scanner using HM-11s
 */
#include <EEPROM.h>
...
void setup() {
  ...
  pinMode(FIN, OUTPUT);
  pinMode(RIN, OUTPUT);
  //2018-08-20 조소장님 지시사항 사용안함 pinMode(HALL_IC_VOUT, INPUT_PULLUP);
  //2018-08-20 조소장님 지시사항 사용안함 pinMode(Close, INPUT_PULLUP);
  //2018-08-20 조소장님 지시사항 사용안함 pinMode(Open, INPUT_PULLUP);
  ...
  //delay(200);
  //blueToothSerial.print("AT+BAUD0");  // HM-11 기준임 0 : 9600 복사품 ==정품은 = 4
  //delay(200);
  //blueToothSerial.begin(9600);
  //delay(200);
  ...
  //Serial.print("init=");
  //Serial.println(adcInItVal);
  //TEST 추후 삭제 요망 닫힘상태로 만들어주기 ============== START
  //Serial.println(Open_flag);
  /* === START === 2018-08-20 조소장님 지시사항 사용안함
...
void loop() {
  ...
  if(btOpen){
    ...
    if(adcOnOff){
    //2018-08-20 조소장님 지시사항 사용안함 //   if(Open_flag == false && sensorValue < (adcInItVal-20)){
      if(sensorValue < (adcInItVal-120)){
  ...
  */
  if(blueToothSerial.available()){
      ...
      if(newUserYn){  //신규유저등록 신호 확인후 등록
        ...
        if(userYn){
          if(readBtUUid == 'B'){	//ADC 인식 배제 로직 추가(UUID만 일치하면 DOOR OPEN) 20180821 조소장님 요청사항
            adcOnOff = false;
          }
      ...
      //Serial.print(readBtUUid);
      
      //=== START ======== 도어락 닫힘 로직 추가(APP에서 닫힘 버튼 추가)20180821 조소장님 요청사항 ========
      if(readBtUUid == 'F'){  
        uuidCloseCnt++;
      }else{
        uuidCloseCnt = 0;
