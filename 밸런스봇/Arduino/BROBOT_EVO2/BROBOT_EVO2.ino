...
void setup()
{
  ...
  delay(100);
  ESPsendCommand("AT", "OK", 1);                                        // AT 확인
  ESPsendCommand("AT+RST", "OK", 2); // ESP Wifi module RESET           // AT 리셋
  ESPwait("ready", 6);
  ESPsendCommand("AT+GMR", "OK", 5);                                    // 펌웨어 버전 정보   

#ifdef EXTERNAL_WIFI
  ESPsendCommand("AT+CWQAP", "OK", 3);                                  // AP와 연결 해제
  ESPsendCommand("AT+CWMODE=1", "OK", 3);                               // 모듈 클라이언트 모드 
  //String auxCommand = (String)"AT+CWJAP="+WIFI_SSID+","+WIFI_PASSWORD;
  char auxCommand[90] = "AT+CWJAP=\"";                                  // AP에 연결
...
#ifdef WIFI_IP
  strcpy(auxCommand, "AT+CIPSTA=\"");                                   // 스테이션 주소 설정
  strcat(auxCommand, WIFI_IP);
  strcat(auxCommand, "\"");
  ESPsendCommand(auxCommand, "OK", 4);
#endif
  ESPsendCommand("AT+CIPSTA?", "OK", 4);                                // 스테이션 주소 확인
#else  // Deafault : we generate a wifi network ########### 기본셋팅 처음 진행되는곳
  Serial1.println("AT+CIPSTAMAC?");                                   // AP 맥 주소 확인
  ESPgetMac();
  //Serial.print("MAC:");
  //Serial.println(MAC);
  delay(200);
  ESPsendCommand("AT+CWQAP", "OK", 3);                                  // AP와의 연결 해제
  ESPsendCommand("AT+CWMODE=2", "OK", 3); // Soft AP mode               // AP 호스트 모드
  // Generate Soft AP. SSID=JJROBOTS, PASS=87654321
  char *cmd =  "AT+CWSAP=\"RIONSOFT_XX\",\"87654321\",5,3";             // AP 주소 설정
  // Update XX characters with MAC address (last 2 characters)
  cmd[19] = MAC[10];
  cmd[20] = MAC[11];
  ESPsendCommand(cmd, "OK", 6);
#endif
  // Start UDP SERVER on port 2222, telemetry port 2223     ########### 기본셋팅 이후 진행되는곳
  Serial.println("Start UDP server");
  ESPsendCommand("AT+CIPMUX=0", "OK", 3);  // Single connection mode    // AP단일 연결 모드
  ESPsendCommand("AT+CIPMODE=1", "OK", 3); // Transparent mode          // AP 미가공 전송 모드
  char Telemetry[80];
  strcpy(Telemetry,"AT+CIPSTART=\"UDP\",\"");                           // UDP포트 등록 및 연결 시작
  ...
  ESPsendCommand("AT+CIPSEND", ">", 2); // Start transmission (transparent mode)  // 데이터 전송
